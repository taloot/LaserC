...
class ROI(GraphicsObject):
    add_segment_signal = pyqtSignal(list, int)
    cursor_move_signal = pyqtSignal(int, float, float)
    """
    ...
    **Signals**
    ...
    def myaddFreeHandle(self, pos=None, axes=None, item=None, name=None, index=None):
        
        if pos is not None:
            pos = Point(pos)
        return self.myaddHandle({'name': name, 'type': 'f', 'pos': pos, 'item': item}, index=index)
    ...
    def myaddHandle(self, info, index=None):
        ## If a Handle was not supplied, create it now
        if 'item' not in info or info['item'] is None:
            h = Handle(0, typ=info['type'], pen=self.handlePen, parent=self)
    ...
    def checkPointMove(self, handle, pos, modifiers):
        ...
        """
        index = self.indexOfHandle(handle)     
        p0 = self.getHandles()[index].pos()
        #p0 = self.mapToParent(h['pos'] * self.state['size']) 
        self.cursor_move_signal.emit(index,p0.x(),p0.y())
        return True

    def mymovePoint(self, index, pos, modifiers=QtCore.Qt.KeyboardModifier(), finish=True, coords='parent'):
        self.handles[index]['item'].setPos(pos)
        self.handles[index]['pos'] = pos
        index1 = 2
        if index == 1:
            index1 = 3

        self.handles[index1]['item'].setPos(Point(index*100,pos.y()))
        self.handles[index1]['pos'] = Point(index*100, pos.y())
    ...
    def movePoint(self, handle, pos, modifiers=QtCore.Qt.KeyboardModifier(), finish=True, coords='parent'):
        ...
        if h['type'] == 't':
            ...
            if index == 0:
                if newPos.x() > self.handles[1]['pos'].x():
                    newPos.setX(self.handles[1]['pos'].x())
                if newPos.x() <= 0:
                    newPos.setX(0)
                if newPos.y() > self.handles[1]['pos'].y():
                    newPos.setY(self.handles[1]['pos'].y())
                if newPos.y() <= 0:
                    newPos.setY(0)
                h1['item'].setPos(Point(0,newPos.y()))
                h1['pos'] = Point(0, newPos.y())
            if index == 1:
                if newPos.x() < self.handles[0]['pos'].x():
                    newPos.setX(self.handles[0]['pos'].x())
                if newPos.x() >= 100:
                    newPos.setX(100)
                if newPos.y() < self.handles[0]['pos'].y():
                    newPos.setY(self.handles[0]['pos'].y())
                if newPos.y() >= 100:
                    newPos.setY(100)
                h2['item'].setPos(Point(100,newPos.y()))
                h2['pos'] = Point(100, newPos.y())
...
class Handle(UIGraphicsItem):
    ...
    def movePoint(self, pos, modifiers=QtCore.Qt.KeyboardModifier(), finish=True):
        ...
        #for r in self.rois:
        self.rois[0].movePoint(self, pos, modifiers, finish=finish, coords='scene')
    ...
    **Arguments**
    ...
    def setPoints(self, points, closed=None):
        ...
        **Arguments**
        ...
        self.myaddFreeHandle((0, points[0][1]))
        self.myaddFreeHandle((100, points[-1][1]))
        self.addSegment(self.handles[0]['item'], self.handles[-2]['item'])
        self.addSegment(self.handles[-3]['item'], self.handles[-1]['item'])
